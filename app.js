...
// ---------- Theme (Tết mode) ----------
function isTetModeFromLunar(lunarDay, lunarMonth){
  const d = parseInt(lunarDay, 10);
  const m = parseInt(lunarMonth, 10);
  if (!d || !m) return false;
  // Bật từ 23/12 âm -> hết tháng 1 âm
  return (m === 12 && d >= 23) || (m === 1);
}

function applyThemeByLunar(lunarDay, lunarMonth){
  const link = document.getElementById("theme-css");
  if (!link) return;
  const wantTet = isTetModeFromLunar(lunarDay, lunarMonth);
  const current = link.getAttribute("href") || "";
  const next = wantTet ? "style-tet.css" : "style.css";
...
function renderBlockForRow(row, tz){
  ...
  applyThemeByLunar(row.am_day, row.am_month);
